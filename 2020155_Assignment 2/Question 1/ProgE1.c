...
void sigalrm_handler(int signum){
	int len = strlen("Inside the SIGALRM's signal handler of E1\n");
	write(fileno(stdout), "Inside the SIGALRM's signal handler of E1\n", len);
  sleep(2);
}
...
int main(int argc, char const *argv[])
{
  sleep(2);
  printf("\nInside program E1\n");
  printf("Destination Process ID for invoking SIGTERM from inside program E1: %d\n", atoi(argv[0]));
