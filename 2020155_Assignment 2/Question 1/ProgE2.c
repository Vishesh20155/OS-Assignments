...
void sigalrm_handler(int signum){
	int len = strlen("Inside the SIGALRM's signal handler defined in program E2\n");
	write(fileno(stdout), "Inside the SIGALRM's signal handler defined in program E2\n", len);
  ...
  printf("No of clock cycles from inside the SIGALRM handler of ST = %llu\n", x);
  printf("Time inseconds elapsed since last reset: %llu seconds\n", x/1800000000);
  time_t t = time(NULL);
